...
# PROBLEMS WITH THE CODE:
    # 1) it does not fully support UTF-8 encoding, because python 2.7 uses ascii... so items used in
        #legal citation website, such as &sect; generate an error, however the output is still
        #generated successfully and as expected.
    # 2) The Table of Contents must be generated or we have an error
    # 3) The TOC is generated incorrectly, must be changed
...
# HOW TO USE THE CODE:
    ...
    # python web2epub.py -t "Basic Legal Citation" -a "Martin, Peter W." -c CoverImage.gif -o LegalCitation.epub index.html 1-100.html...
...
if __name__ == '__main__':
    ...
    #title above cover image removed
    ...
    # we obviously need to change this to something replaceable
    stylesheet_tpl = open("citation.css").read()
    ...
    for i,url in enumerate(args):
        ...
        htmltitle = BeautifulSoup(html)
        htmltitle = htmltitle.title.string

        manifest += '<item id="article_%s" href="article_%s.html" media-type="application/xhtml+xml"/>\n' % (i+1,i+1)
        spine += '<itemref idref="article_%s" />\n' % (i+1)
        toc += '<navPoint id="navpoint-%s" playOrder="%s"> <navLabel> <text>%s</text> </navLabel> <content src="article_%s.html"/> </navPoint>' % (i+2,i+2,htmltitle,i+1)
